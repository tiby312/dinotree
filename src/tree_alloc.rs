...
pub struct NodeConstructor<N,T:HasAabb,I:TrustedLen<Item=T>>{
    pub misc:N,
    pub it:I
}
...
impl<'a,N:'a,T:HasAabb+'a> Vistr<'a,N,T>{
    ...
    pub fn create_wrap<'b>(&'b mut self)->Vistr<'b,N,T>{
        Vistr{ptr:self.ptr,height:self.height,depth:self.depth}
    }
}
...
impl<'a,N:'a,T:HasAabb+'a> Visitor for Vistr<'a,N,T>{
    ...
    fn next(self)->(Self::Item,Option<(Self::NonLeafItem,Self,Self)>){
        ...
        if self.depth<self.height-1{
            ...
            let stuff=(nn,Vistr{ptr:&node.next_nodes.0,depth:self.depth+1,height},
            Vistr{ptr:&node.next_nodes.1,depth:self.depth+1,height});
...
impl<'a,N:'a,T:HasAabb+'a> VistrMut<'a,N,T>{
    ...
    pub fn create_wrap_mut<'b>(&'b mut self)->VistrMut<'b,N,T>{
        VistrMut{ptr:self.ptr,height:self.height,depth:self.depth}
    }
}
...
impl<N,T:HasAabb> TreeAllocDstDfsOrder<N,T>{
    ...
    pub fn new<I:TrustedLen<Item=T>>(it:impl compt::Visitor<Item=NodeConstructor<N,T,I>,NonLeafItem=ExtraConstructor<T::Num>>,height:usize,num_nodes:usize,num_bots:usize)->TreeAllocDstDfsOrder<N,T>{
        ...
        impl Counter{
            fn add_leaf_node<N,T:HasAabb,I:TrustedLen<Item=T>>(&mut self,constructor:NodeConstructor<N,T,I>)->Unique<NodeDyn<N,T>>{
            ...
            fn add_non_leaf_node<N,T:HasAabb,I:TrustedLen<Item=T>>(&mut self,constructor:NodeConstructor<N,T,I>,cc:ExtraConstructor<T::Num>)->Unique<NodeDstDyn<N,T>>{
                let len=constructor.it.size_hint().0;
        ...
        fn recc<N,T:HasAabb,I:TrustedLen<Item=T>>
            (it:impl compt::Visitor<Item=NodeConstructor<N,T,I>,NonLeafItem=ExtraConstructor<T::Num>>,counter:&mut Counter)->Unique<Marker<N,T>>{
