...
pub struct Node2<'a,T:HasAabb+'a>{ 
    ...
    pub fullcomp:Option<tree_alloc::FullComp<T::Num>>,
    pub mid:&'a mut [T]
}
    ...
    if depth<height-1{
        ...
        let (node,left,right)=match construct_non_leaf(BinStrat::MidLeftRight,sorter,div_axis,rest){
            Some((fullcomp,left,mid,right))=>{
                ...
                (Node2{fullcomp:Some(fullcomp),mid},left,right)
            },
            None=>{
                //We don't want to return here since we still want to populate the whole tree!
                (Node2{fullcomp:None,mid:&mut []},&mut [] as &mut [T],&mut [] as &mut [T]) //TODO rust should make this easier
            }
        };
        ...
        let mut node=Node2{fullcomp:None,mid:rest};
