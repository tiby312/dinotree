...
pub(crate) fn inner<A:AxisTrait,T:HasAabb>(axis:A,iter:compt::LevelIter<Vistr<T>>)->Result<(),()> {
            ...
            Some(div)=>{
                ...
                match nn.cont{
                    Some(cont)=>{
                        ...
                        assert2!(a_bot_has_value(nn.bots.iter().map(|b|b.get().get_range(axis).left),*div));
                                    
                        for bot in nn.bots.iter(){
                            assert2!(cont.contains_range(bot.get().get_range(axis)));
                        }

                        assert2!(a_bot_has_value(nn.bots.iter().map(|b|b.get().get_range(axis).left),cont.left));
                        assert2!(a_bot_has_value(nn.bots.iter().map(|b|b.get().get_range(axis).right),cont.right));
                    },
                    None=>{
                        assert2!(nn.bots.is_empty())
                    }
                }
