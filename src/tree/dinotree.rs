...
impl<'a,A:AxisTrait,T:Copy,Num:NumTrait,F:FnMut(&T)->Rect<Num>> DinoTreeBuilder<'a,A,T,Num,F>{
    pub fn new(axis:A,bots:&[T],aabb_create:F)->DinoTreeBuilder<A,T,Num,F>{
        ...
        DinoTreeBuilder{axis,bots,aabb_create,rebal_strat,height,height_switch_seq}
    }
    ...
    pub fn build_with_splitter_seq<S:Splitter>(&mut self,splitter:&mut S)->DinoTree<A,BBox<Num,T>>{
    ...
    fn build_inner<JJ:par::Joiner,S:Splitter+Send>(&mut self,par:JJ,sorter:impl Sorter,ka:&mut S)->DinoTree<A,BBox<Num,T>>{
        ...
        let (new_bots,new_tree)={
            ...
            let new_nodes={
                ...
                for node in cont_tree.get_tree_mut().dfs_iter(){
                    ...
                    new_nodes.push(Node{range:b,cont:node.cont,div:node.div})
                }
                new_nodes
            };
